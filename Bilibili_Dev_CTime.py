...
class ChosenTime(UserElement):
    ...
    def collect_area(self):
        try:
            ...
            if response.status_code == 200:
                area = json.loads(response.text)['data']
                return area
            else:
    ...
    def cope_area(self, area, csrf):  # 这里是核心分区的扫描
        tasklist = []
        for i in area:
    ...
    def cycle_page(self, parents_id, child_id, csrf):
        ...
        while True:
            ...
            if page > self.max_page:
                ...
                break
            area_child = self.scanner_page(parents_id, child_id, page)
            if area_child:
                self.scan_page_room(area_child, csrf)
            else:
                break
    ...
    def TX(self, rid, roomid, uid, csrf):
        ...
        try:
            ...
            if response.status_code == 200:
                tianxuan = json.loads(response.text)
                if tianxuan['code'] == 0:
                    if tianxuan['message'] == '':
                        ...
                        self.logger.info(tianxuan['message'])
                else:
                    self.logger.info(tianxuan['message'])
                    return tianxuan['code']
            else:
                self.logger.error('天选失败，状态码：%s' % response.status_code)
        except Exception as e:
            self.logger.error(tianxuan)
